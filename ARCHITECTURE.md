...
│   │   ├── components/
│   ├── components/
│   │   ├── layout/         # 新版布局组件（V0.0.4 新增）
│   │   │   ├── AppLayout.js/scss    # 主布局（左侧导航+顶部栏+底部工具栏+视图Tab）
│   │   │   ├── RightPanel.js/scss   # 右侧面板（ADC双端颜色滑条+实时统计）
│   │   │   └── SitBackView.js/scss  # 坐垫/靠背视图（左2D+右3D）
│   │   ├── three/          # Three.js 3D 可视化组件（14 个）
│   │   ├── chartsAside/    # ECharts 图表侧边栏（DraggablePanel 浮动）
│   │   ├── ColAndHistory/  # 采集历史组件（Drawer 形式）
│   │   ├── viewSetting/    # 视图设置（旧版，已被 AppLayout 替代）
│   │   ├── title/          # 标题栏组件（旧版，已被 AppLayout 替代）
│   │   ├── aside/          # 侧边栏
│   │   ├── Drawer/         # 抽屉组件
│   │   ├── num/            # 数值显示组件
│   │   └── EquipStatus/    # 设备状态组件
...
| 2026-05-08 | 新增功能 | UI 重构 V0.0.4：AppLayout 主布局组件（左侧图标导航/顶部三大按钮/视图Tab/底部工具栏/状态栏） |
| 2026-05-08 | 新增功能 | RightPanel 右侧面板：ADC 0~255 双端可拖拽颜色滑条（上限默认200/下限默认5，持久化到 localStorage，所有视图同步） |
| 2026-05-08 | 新增功能 | SitBackView 坐垫/靠背视图：左侧2D数字矩阵 + 右侧3D热力图，通过视图Tab（3D整体/坐垫/靠背）切换 |
| 2026-05-08 | 优化重构 | equipStore 新增 adcUpper/adcLower 状态替代旧 color 参数，11个3D/2D组件颜色映射改为 adcLower~adcUpper 范围 |
| 2026-05-08 | 优化重构 | 去掉可视化参数中的颜色调节项（已由 ADC 滑条替代），版本号更新至 V0.0.4 |
...
| 2026-05-08 | 优化重构 | UI 全面重构 V0.0.4：新版布局（左侧图标导航+顶部三大按钮+视图Tab+底部工具栏）、ADC 0~255 双端颜色滑条（持久化）、坐垫/靠背独立视图（左2D+右3D）、实时统计面板、去掉可视化参数中的颜色调节项 |
