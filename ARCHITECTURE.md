...
> 本文档由 Manus 自动生成和维护。最后更新于：2026-05-09 20:15
...
17. **2D 数字格子内绘制**
    - `NumThreeColorV2/V3/V4` 的数字纹理绘制统一按单元格裁剪，避免 100 以上三位数绘制到相邻格子。
    - 方形矩阵和非方形靠背矩阵的数字纹理都改为高分辨率 16x16 sprite sheet，数字统一使用固定字号并通过格内裁剪兜底，保证格子视觉大小保持一致。
...
| 2026-05-09 | 2D 数字格内裁剪 | 修复 2D 数字视图三位数超出单位格的问题，数字纹理按格裁剪并保持固定字号 |
...
| 2026-05-09 20:10 | 修复缺陷 | 修复 2D 数字超出单位格的问题：`NumThreeColorV2/V3/V4` 的数字 sprite sheet 增加单元格裁剪和固定字号，方形矩阵与非方形靠背矩阵均保持数字在格内且视觉格子大小一致 |
