...
## 2026-06-01 手动断开清屏与导出命名去 endi
- `Title.disconnect()` 手动断开后会同步调用页面级 `clearVisualizationData()`，清空设备在线状态、MAC、数据质量、实时矩阵、COP、历史图表、回放状态和采集状态；`Test` 在 `connectState=idle` 时会忽略断开后的残留实时帧，避免设备状态和图表被最后一包数据重新写回。
- `/stopPort` 后端接口除关闭串口外，同步退出历史/对比回放状态，清空 `historyDbArr/leftDbArr/rightDbArr/historySelectCache` 并停止回放 timer，避免断开后后端仍保留历史播放上下文。
- `util/db.js` 的历史导出文件名通过 `toPublicExportName()` 将 `endi` 转为公开名称 `car`；`NumThresContrast` 的对比 CSV 文件名和 `object` 字段将 `endi-back/endi-sit` 显示为 `back/sit`，不再把内部系统 key 暴露给用户。
| 2026-06-01 | 修复缺陷 | 修复手动断开后传感器状态、数据和实时图表未清空，并移除对比/导出 CSV 命名中的 endi 字样 |
...
- 固定字段中的单个“设备MAC”拆分为“靠背MAC”和“座椅MAC”，导出时分别从靠背/坐垫矩阵帧读取，导入时也按矩阵类型回填对应 MAC。
| 2026-06-01 | 优化重构 | 将历史数据下载改为第一行表头展开对象，字段选择仍保持一套通用字段 |

## 2026-06-01 历史播放结束进度条保留
- `Test.onPlayEnd()` 在历史回放自然结束时不再重置 `history.index/timestamp` 和回放记录信息，只停止播放状态并按需清理可视化矩阵，避免底部进度条回到 0 或立即消失。
- `clearVisualizationData()` 增加 `preservePlaybackStatus` 参数，断开连接后播放结束仍可清空画布数据，但保留当前历史播放进度和时间戳。
- `ColAndHistory` 的播放控件显示条件从仅依赖 `dataStatus === 'replay'` 调整为只要当前存在选中的历史回放记录就保留，直到用户主动点击历史入口退出。
| 2026-06-01 | 修复缺陷 | 修复历史数据播放完后底部进度条自动消失的问题 |

## 2026-06-01 采集框选持久化与回放重播
- `Col` 在采集开始和停止时读取当前 `brushInstance.rangeArr`，按矩阵 key 转换为 `{ regions: [...] }` 结构并写入 `remarks.select_json`，因此不退出框选模式也能在历史下载中导出框选区域数据。
- 采集停止时会先完成当前框选持久化，再调用 `/endCol` 结束采集，避免用户立刻进入历史下载时读到尚未保存的框选信息。
- `PlaybackBar` 在历史回放结束后再次点击播放时，会先把前端状态切回 `replay` 并把结束态进度重置到第 0 帧，再启动后端回放，保证再次播放从头开始。
| 2026-06-01 | 修复缺陷 | 修复采集中未退出框选导致历史下载缺少框选数据，以及历史回放结束后再次播放未从第 0 帧开始的问题 |

## 2026-06-01 靠背倍率、颜色上限与 COP 报告显示修正
- `SerialManager` 取消 `estimatePointPressure` 单点压强公式接入，恢复坐垫原始线序转换；所有 `*-back` 靠背矩阵在线序转换后统一对每个点乘以 `1.8`，并继续走原有实时、采集、回放和导出链路。
- 可视化参数的颜色默认值调整为 `180`、最大值保持 `255`；后端读取、保存和生成系统配置时会强制归一 `optimalObj.*.color=180`、`maxObj.*.color=255`，避免旧配置继续返回旧色值。
- COP 报告接口返回首帧 `collectedAt`，前端将“采集时间”格式化为日期时间；设备名称、传感器类型和报告内传感面显示改为公开名称，不再暴露 `endi-back/endi-sit`。
| 2026-06-01 | 修复缺陷 | 取消单点压强公式，靠背点值统一乘 1.8，颜色最大值改为 255，并修正 COP 报告时间和名称显示 |
...
- 前端默认配置、系统设置兜底配置和 zustand 初始化兜底值中的颜色默认值统一调整为 `180`，`maxObj.*.color` 继续保持 `255` 作为颜色上限。
...
## 2026-06-03 2D 数字色阶同步
- `client/src/components/three/NumThreeColorV2.js` 的 2D 数字贴图由固定 `0..22` 色阶改为根据当前“颜色调节”值动态生成，拖动颜色调节时会替换贴图并释放旧纹理。
- 2D 数字显示贴图容量从 `0..255` 扩展到 `0..4095`，避免靠背点值乘数放大后数字被截断为 255，保证调节面板的当前最大值与 2D 数字画布更一致。
- `SecondTitle` 的“颜色调节-当前最大值”不再直接取原始 `displayStatus` 最大值，而是按当前坐垫/靠背视图尺寸应用噪点过滤和 2D 数字同款高斯平滑后再统计，避免出现画布最大值 73、面板最大值 117 这类未平滑/已平滑数据不一致。
| 2026-06-03 | 修复缺陷 | 修复 2D 数字色阶和可视化调节最大值显示不一致 |

## 2026-06-03 框选区域列表操作精简
- `SelectSet` 的框选区域卡片移除无实际操作的眼睛图标按钮，仅保留删除按钮和区域名称编辑入口。
- 同步删除 `selectRegionEyeButton` 专用样式，避免无用样式残留。
- 框选区域卡片从四列改为三列布局，恢复卡片上下 padding，并让删除按钮右对齐以贴近卡片右侧。
| 2026-06-03 | 交互优化 | 移除框选内容框的眼睛图标按钮 |

## 2026-06-03 框选模板重名覆盖确认
- `SelectSet.saveTemplateByName` 在保存模板前检测同名模板；存在重名时先弹出确认框，用户确认后才覆盖原有模板。
- `App.js` 补充中英文文案 `templateNameExists` 与 `overwriteTemplateConfirm`，避免弹窗显示翻译 key。
| 2026-06-03 | 交互优化 | 框选模板重名时增加覆盖确认弹窗 |
