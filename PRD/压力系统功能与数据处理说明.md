...
`filter` 配置、接口和采集快照字段暂时保留以兼容现有界面与历史数据，但 `filterApplied=false`，用户配置不参与计算。座椅和靠背在插值、方向修正和置零后执行 V2.7.63 固定门槛：有限且 `ADC > 30` 的点参与标定，`ADC <= 30` 或无效的点保持为 0。
...
当前直接启用 `server/kpa/adc-matrix-to-pressure-filter30-v2.7.63.js`。应用不复刻逐点标定规则，而是将插值后的二维 ADC 矩阵和传感器类型直接交给公式文件：

```js
const seatResult = PressureCalibrationExport.calculatePressureFromMatrix(
  seatAdcMatrix,
  { sensorType: "seat", humanCoefficient: 2.2 },
)
const backrestResult = PressureCalibrationExport.calculatePressureFromMatrix(
  backrestAdcMatrix,
  { sensorType: "backrest", humanCoefficient: 2.2 },
)
```

两个调用结果中的 `pressureMatrixKPa` 分别作为坐垫和靠背的标准压强矩阵；分支判断、归一化和真人系数均由标定文件内部完成。

进入公式前，矩阵完成线序/后端插值、方向修正和置零。V2.7.63 在文件内部完成有限值处理和固定 ADC 30 门槛；原始有效点定义为有限且 `ADC_i > 30`，`ADC_i <= 30` 在最终矩阵中保持为 0。
...
- 座椅 V2.7.63：16 段线性曲线；首段以下按过原点斜率计算；末段上界 `173.74` 以上基础压强固定为 `27 kPa`。
- 靠背 V2.7.63：18 段线性曲线；首段以下按过原点斜率计算；末段上界 `189.43` 以上基础压强固定为 `27 kPa`。
- 传感面判定：key 含 `back` 使用靠背曲线，key 含 `sit`/`seat` 使用座椅曲线。
- 应用仅将映射明确的 `seat` / `backrest` 传感面送入当前标定文件，不把其他 ADC 矩阵当作 kPa 兜底。
...
| 水印版本 | `backend-zero-native-v16-matrix-calibration-v2763` | 判断是否需按 V2.7.63、`ADC<=30` 门槛、整帧标定 API 与有效点掩码统计口径补算 |
