...
- 实现步骤（`SerialManager.js:827` `bindDataHandler`）
	...
	4. 座椅与靠背标定输入口径
		...
		- 原生座椅和靠背统一以有限且 `ADC>=30` 的点作为标定有效点；低于 30 的值在调用标定文件前置为 0
...
- 实现步骤
	...
	4. 压强换算（`pressureFrameProcessor.js` 的 `calcPressureFormulaStats`）
		...
		- 当前配置为 `point_pressure_calibration.js`
		- 原生标定接口为 `calculateBasePressure / getCalibrationInput / calculateWeightPointPressures / calculatePressureMetrics / adcMatrixToPressureMatrix`
		- 运行时直接加载业务提供的 `point_pressure_calibration.js`；应用直接消费 `adcMatrixToPressureMatrix()` 返回的完整矩阵
		- 坐垫调用 `adcMatrixToPressureMatrix(seatAdcMatrix, "seat", 2.2)`，靠背调用 `adcMatrixToPressureMatrix(backrestAdcMatrix, "backrest", 2.2)`
...
- 关键常量
	- `PROCESSING_VERSION = 'backend-zero-native-v15-matrix-calibration-min30'`
	- `ADC_PREPROCESSING_MODE = 'zero-baseline-native-min30'`
	- `NATIVE_CALIBRATION_MIN_ADC = 30`
	- `NATIVE_CALIBRATION_DISTRIBUTION = 'native-adc-matrix-to-pressure-matrix-v2746-v2752'`
...
- 代码位置
	...
	- `server/kpa/point_pressure_calibration.js`
...
- 旧历史兼容
	- 缺少标准矩阵时由后端补算一次
	- 仅带 `processing.version = 'backend-zero-native-v15-matrix-calibration-min30'` 且 ADC 预处理、公式、分配算法及有效点掩码水印一致的新数据不重复处理
- 代码位置
...
| `PROCESSING_VERSION` | `backend-zero-native-v15-matrix-calibration-min30` | `pressureFrameProcessor.js` | 处理版本水印 |
| `ADC_PREPROCESSING_MODE` | `zero-baseline-native-min30` | `pressureFrameProcessor.js` | 标识原生座椅与靠背仅保留有限且不低于 30 的 ADC |
| `NATIVE_CALIBRATION_MIN_ADC` | 30 | `pressureFrameProcessor.js` | 调用标定文件前的固定 ADC 下限；30 保留，低于 30 置零 |
