...
sequenceDiagram
    ...
    FP->>FP: 有限值与非负归一化
    FP->>FP: 按标定文件规则补齐完整 kPa 矩阵 → 压力换算
    FP-->>DS: arr / calibrationAdcArr / pressureArr / forceArr / processing
...
- 实现步骤（`SerialManager.js:827` `bindDataHandler`）
	...
	4. 座椅与靠背标定输入口径
		- 靠背线序转换和插值完成后，矩阵 ADC 原值进入置零链路，不再乘靠背专属倍率
		- 原生座椅和靠背统一以有限且 `ADC>30` 的点作为标定有效点；V2.7.63 将 30 及以下的值置为 0
...
- 实现步骤
	...
	5. 特殊有效区
		...
		- 无传感器填充位由 `endiBack1024` 的矩阵映射直接输出 0
		- 这是设备点位拓扑，不是压强规则；标准处理器不再按行列二次清除 ADC
- 输出
	- 方向统一的标准 ADC 矩阵
...
- 输入
	...
	- 当前处理参数快照（`filter/gauss/coherent` 仅用于接口与历史配置兼容）
- 实现步骤
	1. 记录兼容参数（`DataService.js` `getActiveFrameProcessingConfig`）
		...
		- `normalizeFrameProcessingConfig` 做范围钳制
			...
			- `gauss` 0 ~ 4，默认 2
			- `coherent` 1 ~ 10，默认 1
		- 三个值只写入配置快照和 `processing` 水印，不改变标准 ADC、压强、压力、点数或面积
	2. 保留原始 ADC 并置零（`DataService.js`）
	...
	3. 标定 ADC 归一化（`util/pressureFrameProcessor.js` `normalizeCalibrationAdcMatrix`）
		- 进入本步骤前已在线序阶段完成坐垫 `23×23 → 46×46`、靠背 `25×32 → 50×64` 插值；标定始终使用插值后的完整矩阵
		- 长度与 `width * height` 不符直接返回空数组
		- 逐点转为有限数值并执行非负归一化；有限正数保留，零、负数和非有限数归零
		- 不执行用户可调 ADC 过滤、空间高斯或时序平滑
		- 结果同时写入 `arr` 和 `calibrationAdcArr`，并原样交给标定调用层
	4. 压强换算（`pressureFrameProcessor.js` 的 `calcPressureFormulaStats`）
		- 公式模块由 `PressureConfig.loadPressureFormula()` 动态 `require`，按文件 mtime 缓存
		- 当前配置为 `adc-matrix-to-pressure-filter30-v2.7.63.js`
		- 原生标定接口为 `calculatePressureFromMatrix / adcMatrixToPressureMatrix / filterAdcMatrix / estimateBasePressure / getCalibrationInfo`
		- 运行时直接加载业务提供的 V2.7.63 文件；应用直接消费 `calculatePressureFromMatrix()` 返回的 `pressureMatrixKPa`
		- 坐垫传入 `{ sensorType: "seat", humanCoefficient: 2.2 }`，靠背传入 `{ sensorType: "backrest", humanCoefficient: 2.2 }`
		- 砝码段坐垫按降序第 5-70 位、靠背按 TOP46 计算 `X` 与 `Pbar=F(X)`，逐点使用 `Pᵢ=F(ADCᵢ)×Pbar/AVG(F(ADCᵢ))`
		- 有效点数 `>300` 时保形展开 `Pᵢ=max(0,F(ADCᵢ))×2.2`；恰好 300 点仍走砝码归一化
		- `pressureArr` 保留标定公式完整精度，界面渲染时再保留一位小数
		- `averagePressureKPa = SUM(pressureArrᵢ) / COUNT(calibrationValidMaskᵢ=1)`，`maxPressureKPa = MAX(pressureArrᵢ)`
		- `matrixAveragePressureKPa / matrixMaxPressureKPa` 与上述平均/最大使用同一份 `pressureArr`，不再存在独立整帧摘要口径
		- 热力图、2D 数字、重心、正态分布、点数、面积、框选、压力总和、导出和 COP 报告统一使用逐点 `pressureArr / forceArr`
		- 旧版 `master` 和 `estimatePressure / estimateMaxPressure` 公式仍保留兼容分支
		- 传感面判定 `getPressureSensor`：key 含 `back` → `backrest`，含 `sit` 或 `seat` → `seat`
		- 当前原生标定未定义的传感器 key 直接报错，不再返回原始 ADC 作为伪 kPa
		- 输出单位 `kPa`
	5. 压力换算
		...
		- 输出单位 `N`
	6. 精度与元数据
		- 标定返回矩阵必须与 ADC 等长且全部为有限数值，否则抛出错误，不静默增加压强规则
		- `DISPLAY_DIGITS = 1` 只描述界面/导出展示精度，不回写并改变标准矩阵
		- `buildProcessingMetadata` 生成 `processing`
			- `version = 'backend-zero-native-v16-matrix-calibration-v2763'`
			- `adcPreprocessing = 'zero-baseline-native-filter30-v2763'`
			- `calibrationInputMinAdc = 30`（有效性规则为 `ADC>30`）
			- `filter / gauss / coherent` 仅记录请求值
			- `filterApplied = false`
			- `gaussianApplied = false`
			- `gaussianSigma = 0`
			- `temporal = false`
			- `outputDigits = 1`
			- `formulaFile / formulaProfile / pressureDistribution`
		- 历史标准矩阵只有在版本、ADC 预处理口径、公式文件、公式 profile 和分配算法全部匹配时才直接复用
		- 需要重算时优先使用 `calibrationAdcArr`，其次使用 `rawAdcArr`
	7. 采集期兼容参数锁定
		...
		- `GET /endCol` 解锁并清空快照
		- 锁定行为为接口兼容保留，不代表这些参数参与新标准帧计算
- 关键常量
	- `PROCESSING_VERSION = 'backend-zero-native-v16-matrix-calibration-v2763'`
	- `ADC_PREPROCESSING_MODE = 'zero-baseline-native-filter30-v2763'`
	- `NATIVE_CALIBRATION_MIN_ADC = 30`
	- `NATIVE_CALIBRATION_DISTRIBUTION = 'native-adc-matrix-to-pressure-filter30-v2763'`
	- `HUMAN_VALID_POINT_THRESHOLD = 300`、`DEFAULT_HUMAN_COEFFICIENT = 2.2`（读取自标定文件）
	- `DISPLAY_DIGITS = 1`
	- `GAUSSIAN_SIGMA_FACTOR = 0.5` 与 `gaussianBlur` 仅保留给兼容调用，标准帧链路不调用
	- 默认兼容参数 `{ filter: 30, gauss: 2, coherent: 1 }`
	- `AVERAGE_FORMULA_MIN_ADC = 30` 仅用于兼容旧平均公式；当前原生公式使用置零后的正数点
- 输出（每个传感面）
	- `rawAdcArr` 方向化后的原始 ADC
	- `arr` 置零后仅做有限值与非负归一化的 ADC
	- `calibrationAdcArr` 实际进入标定的插值后 ADC 快照
	- `calibrationValidMask` 记录门槛后的有效位置，有限且 `ADC>30` 的位置为 1
	- `pressureArr` 为标定文件砝码归一化或真人段规则输出的完整精度逐点矩阵，单位 `kPa`
	- `forceArr` 由 `pressureArr × pointAreaCm2 × 0.1` 逐点生成并保留完整精度，单位 `N`
	- `calibrationDiagnostics.averagePressureKPa / maxPressureKPa` 直接从最终热力图 `pressureArr` 计算
	- `calibrationDiagnostics.matrixAveragePressureKPa / matrixMaxPressureKPa` 同一逐点矩阵的诊断别名
	- `processing` 处理元数据
	- `pressureUnit = 'kPa'`、`forceUnit = 'N'`
	- `zeroState` 置零元数据
- 处理参数语义
	- `filter`、`gauss`、`coherent` 都是旧配置兼容字段，不参与标准帧计算
- 页面渲染
	- 压强模式逐点用 `pressureArr`，单位 `kPa`
	- 压力模式逐点用 `forceArr`，单位 `N`
	- 压力总和始终基于 `forceArr`，始终显示 `N`
	- 无框选和存在框选时，左侧平均/最大值都直接使用当前范围内的热力图逐点矩阵统计
...
- 代码位置
	...
	- `server/kpa/adc-matrix-to-pressure-filter30-v2.7.63.js`
...
- 旧历史兼容
	- 缺少标准矩阵时由后端补算一次
	- 仅带 `processing.version = 'backend-zero-native-v16-matrix-calibration-v2763'` 且 ADC 预处理、公式、分配算法及有效点掩码水印一致的新数据不重复处理
- 代码位置
...
- `filter` / `gauss` / `coherent` 兼容字段
	- 仍在配置、快照、接口与 `processing` 元数据中传递
	- `normalizeCalibrationAdcMatrix` 不读取它们；三者都不参与标准 ADC、压强或压力计算
- 两处实时定时器间隔不一致
...
| `arr` | `normalizeCalibrationAdcMatrix` | 置零后仅做有限值与非负归一化的 ADC |
| `calibrationAdcArr` | `processMatrixItem` | 实际进入标定的插值后 ADC 快照 |
| `pressureArr` | `buildCanonicalMetricArrays` | 按标定文件规则展开的完整 kPa 矩阵一位小数序列化结果 |
...
| `PROCESSING_VERSION` | `backend-zero-native-v16-matrix-calibration-v2763` | `pressureFrameProcessor.js` | 处理版本水印 |
| `ADC_PREPROCESSING_MODE` | `zero-baseline-native-filter30-v2763` | `pressureFrameProcessor.js` | 标识原生座椅与靠背仅保留有限且大于 30 的 ADC |
| `NATIVE_CALIBRATION_MIN_ADC` | 30 | `pressureFrameProcessor.js` | 与 V2.7.63 一致的 ADC 门槛；30 及以下置零 |
| `DISPLAY_DIGITS` | 1 | `pressureFrameProcessor.js` | 输出小数位 |
| `GAUSSIAN_SIGMA_FACTOR` | 0.5 | `pressureFrameProcessor.js` | 兼容高斯工具常量，标准帧链路不使用 |
| 默认处理参数 | `filter 30 / gauss 2 / coherent 1` | `state.js` | 旧接口与采集快照兼容值，不参与标准帧计算 |
| 原生标定输入 ADC | `> 0` | `pressureFrameProcessor.js` | 坐垫和靠背共用标定文件的有限正数有效点规则 |
