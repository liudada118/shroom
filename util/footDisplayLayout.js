/**
 * footDisplayLayout.js — 下身热图「一个格子拆成多格」的显示布局（Node 侧）
 *
 * 浏览器侧镜像文件：client/src/util/footDisplayLayout.js
 * 两边逻辑必须保持一致，修改时请同步。
 *
 * 【背景】
...
const ENDI_FOOT_COMBINED_WIDTH = 24
const ENDI_FOOT_SINGLE_WIDTH = 12
const ENDI_FOOT_HEIGHT = 64

// 显示尺寸 = 规范尺寸 × 2（横向 12→24 / 24→48，纵向 64→128）
const FOOT_DISPLAY_SCALE_X = 2
const FOOT_DISPLAY_SCALE_Y = 2
const FOOT_SINGLE_DISPLAY_WIDTH = ENDI_FOOT_SINGLE_WIDTH * FOOT_DISPLAY_SCALE_X
const FOOT_DISPLAY_HEIGHT = ENDI_FOOT_HEIGHT * FOOT_DISPLAY_SCALE_Y
...
function getFootLayoutKind(key) {
...
/** 该部位规范数组的宽度 */
function getFootCanonicalWidth(kind) {
 ...
 */
function getFootDisplayWidth(key, canonicalWidth) {
 ...
 */
function getFootDisplayHeight(key, canonicalHeight) {
  const kind = getFootLayoutKind(key)
...
function visualRowToCanonicalRow(row) {
  return Math.floor(row / FOOT_DISPLAY_SCALE_Y)
}

/** 规范行 → 它占的第一个显示行 */
function canonicalRowToVisualRow(row) {
  return row * FOOT_DISPLAY_SCALE_Y
}

/** 这一规范行属于哪一段 */
function getFootBand(canonicalRow) {
 ...
 */
function visualColToCanonicalCol(visualRow, col, kindOrKey) {
 ...
 * @param {string} kindOrKey getFootLayoutKind 的结果或部位 key
 */
function canonicalColToVisualCol(canonicalRow, canonicalCol, kindOrKey) {
  const kind = normalizeFootKind(kindOrKey)
  if (!kind) return -1
...
function isFootVisualNullCell(visualRow, col, key) {
  const kind = getFootLayoutKind(key)
 ...
 */
function expandFootVisualArr(arr, key) {
  const kind = getFootLayoutKind(key)
 ...
 */
function footVisualRectToCanonicalRect(rect, key) {
  const kind = getFootLayoutKind(key)
...
module.exports = {
  ENDI_FOOT_COMBINED_WIDTH,
  ENDI_FOOT_SINGLE_WIDTH,
  ENDI_FOOT_HEIGHT,
  FOOT_DISPLAY_SCALE_X,
  FOOT_DISPLAY_SCALE_Y,
  FOOT_SINGLE_DISPLAY_WIDTH,
  FOOT_DISPLAY_HEIGHT,
  getFootLayoutKind,
  isEndiFootKey,
  getFootCanonicalWidth,
  getFootDisplayWidth,
  getFootDisplayHeight,
  visualRowToCanonicalRow,
  canonicalRowToVisualRow,
  getFootBand,
  visualColToCanonicalCol,
  canonicalColToVisualCol,
  isFootVisualNullCell,
  expandFootVisualArr,
  footVisualRectToCanonicalRect,
}
