/**
 * gradientMetrics.js — 对称系数 / 压力梯度 计算（Node 侧）
 *
 * 浏览器侧镜像文件：client/src/util/gradientMetrics.js
 * 两边逻辑必须保持一致，修改时请同步。
 *
 * 【对称系数】
 *   部位数据左右平均各分一半，按中线对折，相叠加的点为对称点。
 *   每行：分子 = Σ|左点 − 右点|，分母 = Σ(左点 + 右点)，行系数 = 1 − 分子/分母。
 *   最终系数 = 有效行系数之和 / 有效行数（整行为 0 的行跳过，不计入行数）。
 *
 * 【压力梯度】
 *   grad p = sqrt((∂p/∂x)² + (∂p/∂y)²)
 *   ∂p/∂x = (P右 − P左) / (2d)，∂p/∂y = (P下 − P上) / (2d)，d = 1.25 cm。
 *   仅内部点（去掉最外一圈）参与计算，且要求该点压力 > 0。
 *   平均梯度 = 所有有效点梯度之和 / 有效点数；最大梯度 = 有效点中的最大值。
 *   压强单位统一换算为 pa（1 kPa = 1000 pa），距离单位 cm，故梯度单位为 pa/cm。
 */

// ─── 单位 ────────────────────────────────────────────────
const GRADIENT_UNIT_PA_CM = 'pa/cm'
const GRADIENT_UNIT_N_CM3 = 'N/cm³'
const GRADIENT_UNIT_SEQUENCE = [GRADIENT_UNIT_PA_CM, GRADIENT_UNIT_N_CM3]
// 1 N/cm³ = 10000 pa/cm
const PA_CM_PER_N_CM3 = 10000
// 传感器点间距 12.5 mm = 1.25 cm
const GRADIENT_POINT_SPACING_CM = 1.25
...
function getNextGradientUnit(unit) {
...
/** 不同量级下保留合适的小数位 */
function formatGradientValue(paPerCm, unit) {
...
function supportsSymmetryCoefficient(key) {
  return ['jacket', 'foot'].includes(getMatrixPartName(key))
}

/** 压力梯度 上身 / 下身 / 左臂 / 右臂 都有 */
function supportsPressureGradient(key) {
 ...
 * @param {number[]} values 行优先展开的矩阵数据
 */
function calcSymmetryCoefficient(values, width, height) {
 ...
 * @returns {{ average: number|null, max: number|null, count: number }}
 */
function calcPressureGradientStats(pressureKpaValues, width, height, spacingCm = GRADIENT_POINT_SPACING_CM) {
  const cols = Math.floor(Number(width) || 0)
  const rows = Math.floor(Number(height) || 0)
...
/**
 * 一个部位一帧的完整指标
 * @returns {{ symmetry: number|null, avgGradient: number|null, maxGradient: number|null, gradientPoints: number }}
 */
function calcPartShapeMetrics(pressureKpaValues, width, height, key) {
...
module.exports = {
  GRADIENT_UNIT_PA_CM,
  GRADIENT_UNIT_N_CM3,
  GRADIENT_UNIT_SEQUENCE,
  PA_CM_PER_N_CM3,
  GRADIENT_POINT_SPACING_CM,
  normalizeGradientUnit,
  getNextGradientUnit,
  convertGradientValue,
  formatGradientValue,
  supportsSymmetryCoefficient,
  supportsPressureGradient,
  calcSymmetryCoefficient,
