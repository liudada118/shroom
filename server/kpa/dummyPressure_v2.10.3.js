 ...
 *
 * 浏览器用法：
 *   <script src="dummyPressure_v2.10.3.js"></script>
 *   const result = DummyPressureV2103.calculateDummyMatrixPressure(matrix, "dummy-left-leg-32x6");
 *
 * Node.js CommonJS 用法：
 *   const calibration = require("./dummyPressure_v2.10.3.js");
 *   const result = calibration.calculateDummyMatrixPressure(matrix, "dummy-body-27x12");
 *
 * 计算口径：
 * - 热力图格子显示原始 ADC，本模块只计算平均压强与最大压强。
 * - 有效点必须满足：未被 validMask 排除、数值有限、ADC > 10。
 * - 3 <= N < 100：各部位分别取自身 Top50 ADC 均值并代入自身 master。
 * - N >= 100：每个有效点代入该部位自身 master 后乘以 2 一次，再取均值与最大值。
 * - 上衣/袖子 master：上衣数据九段单调三次 Hermite；左右腿 master：反放砝码全局线性式。
 */
(function attachDummyPressure(root, factory) {
  const api = factory();
  if (typeof module === "object" && module.exports) module.exports = api;
  if (root) root.DummyPressureV2103 = api;
  ...
  const DUMMY_POINTWISE_THRESHOLD = 100;
  ...
  const DUMMY_BODY_MASTER_CONFIG = Object.freeze({
    name: "上衣/袖子九段单调三次 master 公式 (V2.10.3)",
  ...
  const DUMMY_LEG_MASTER_CONFIG = Object.freeze({
    name: "反放砝码全局线性腿部 master 公式 (V2.10.3)",
  ...
  /** 单个 ADC 按反放砝码腿部线性 master 转换为基础压强，返回未四舍五入的 kPa。 */
  function legAdcToKpaRaw(adc) {
    if (!Number.isFinite(adc) || adc <= 0) return 0;
    const pressure = DUMMY_LEG_MASTER_CONFIG.slope * adc + DUMMY_LEG_MASTER_CONFIG.intercept;
  ...
  function runSelfCheck() {
    ...
    [[132.156, 0], [154.024, 7.8], [176.36, 16.45], [185.092, 19.84], [255, 27.5]].forEach(([adc, expected]) => {
      const actual = legAdcToKpa(adc);
      add(`腿部线性 master ADC=${adc}`, equal(actual, expected), actual, expected);
    });
